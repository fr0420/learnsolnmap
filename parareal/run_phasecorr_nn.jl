...
const with_additive = parse(Bool, ARGS[8]);
const output_dir = ARGS[9];
const num_workers = parse(Int, ARGS[10]);
...
function Theta(
    ...
    znew = Omega * construct_z(p, q)
    pnew, qnew = recover_canonical_vars(znew, q)
    return pnew, qnew
end
...
p_all, q_all = Parareal.phasecorr(p0, q0, t, fine_solve, coarse_solve, construct_z, Theta, niters=niters, with_additive=with_additive)
