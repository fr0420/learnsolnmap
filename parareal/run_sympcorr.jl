...
const phi_method = ARGS[10];
const distance_func = ARGS[11];
const with_additive = parse(Bool, ARGS[12]);
const output_dir = ARGS[13];
const num_workers = parse(Int, ARGS[14]);
...
config = OrderedDict(
    ...
    "fine method"=>fine_method, "coarse method"=>coarse_method, "phi method"=>phi_method,
