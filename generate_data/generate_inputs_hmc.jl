...
const dt = parse(Float64, ARGS[5])
const h = parse(Float64, ARGS[6])
const beta = parse(Float64, ARGS[7])
...
@everywhere begin 
    ...
    function rhmc_transition(p::AbstractArray{T, 1}, q::AbstractArray{T, 1}; 
            nsteps::Int, keep_intermediate_states::Bool) where T<:AbstractFloat
...
if with_rejection 
    res = chain_ensemble(p0, q0, (p, q)->rhmc_transition(p, q; nsteps=Nsteps, keep_intermediate_states=keep_itm_states), 
        num_chains=Nchains, num_transitions=Njumps)
else
    res = chain_ensemble(p0, q0, (p, q)->hmc_transition(p, q; nsteps=Nsteps, keep_intermediate_states=keep_itm_states), 
        num_chains=Nchains, num_transitions=Njumps)
end
