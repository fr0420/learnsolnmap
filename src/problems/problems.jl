...
abstract type AutonomousODESystem end
...
"""Compute total energy / Hamiltonian."""
function compute_H(
    prob::AutonomousODESystem, 
    u::AbstractArray{T, 1},
    ) where T<:AbstractFloat
    return compute_energy(prob, u)
end
...
function compute_traj_err(
    prob::AutonomousODESystem,
    sol1::AbstractArray{T, 1}, 
    sol2::AbstractArray{T, 1},
    ) where T<:AbstractFloat
    sol1 = nondimensionalize(prob, sol1)
    sol2 = nondimensionalize(prob, sol2)
    abs_err = norm(sol1 .- sol2)
...
if ARGS == ["--run"]
    ...
    # u0 = initial_condition(prob, Float64x4)
    # println(u0)
    # println(compute_H(prob, u0))

    # du = zero(u0)
    # compute_du!(prob, du, u0)
    # println(du)
    
    # u0_ptb = copy(u0)
    # u0_ptb[1] += 1e-5
    # println(compute_traj_err(prob, u0, u0_ptb))
    # println(compute_H_err(prob, u0, u0_ptb))
    
    v0, x0 = initial_condition(prob, Float64x4)
    @show (v0, x0)
    @show compute_H(prob, v0, x0)
    @show compute_K(prob, v0)
    @show compute_U(prob, x0)

    ddx = zero(x0)
    compute_ddx!(prob, ddx, v0, x0)
    @show ddx

    @show to_slow_fast_variables(prob, v0, x0)
    @show embed_state(prob, (v0, x0))
